...
public class ProblemSet11 {
    ...
    public boolean linearIn(ArrayList<Integer> outer, ArrayList<Integer> inner) {
        try {
            ...
            int lengthO = outer.size();
            int lengthI = inner.size();

            if (lengthO <= 0) return false;
            if (lengthI <= 0) return false;
        ...
        try {
            ...
            for(int i=0; i<length; i++) {
                for(int j=length-1; j>=0; j--) {
                    ...
                    for (int maxI=i; (maxI<length && maxJ>=0); maxI++) {
    ...
    public int countClumps(ArrayList<Integer> numbers) {
        try {
            ...
            for(int i=0; i<length; i++) {
                ...
                for(int j=i+1; (j<length && numbers.get(i+1)==temp); j++) {
                    i++;
                    cLen++;
                }
